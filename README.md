...
This site is running on Firebase.   
To set up on Firebase some changes have been made that mean I can no longer serve the site locally.
So to develope:
Write code
gulp - to rebuild the site
firebase deploy to see the changes live on firebase.

This site is just a straight forward Polymer Starter Kit v.1


It shows some simple ES2015 JavaScript in action 

(Details can be found in the Google CodeLab Write-up.)

To preview the app run `gulp serve`. This will build the app and start serving a development version on http://localhost:5000

OK, finished the tutorial, it works..

Next, load up onto github -> there is a gulp task for deploying to gs-pages...
